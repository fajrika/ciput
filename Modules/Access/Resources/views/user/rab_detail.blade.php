  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Project <strong>{{ $rab->workorder->project->first()->name or '' }}</strong></h1>
          </div>
          <div class="col-sm-6">
          ...
          <div class="card">
            ...
            <div class="card-body col-md-12">
                <table class="table table-bordered">
                  ...
                  <tbody>
                    @foreach ( $rab->units as $key5 => $value5 )
                    <tr>
                      <td>{{ $value5->asset->name }}</td>
                      <td>{{ $value5->asset_type }}</td>
                    </tr>
                    @endforeach
                  </tbody>
                </table><br>
                <h3>Nilai RAB : {{ number_format( $rab->nilai / $rab->units->count(),2 )}} @ {{ $rab->units->count() }} Unit</h3>
              ...
              @if ( count($rab->blok_list) > 0 )   
                ...
                <tbody>
                  @for ( $i=0; $i < count($rab->blok_list); $i++ )
                  <tr>
                    <td><strong>{{ \Modules\Project\Entities\Blok::find($rab->blok_list[$i])->name }}</strong></td> 
                    <td>
                      @php $nilai = 0; @endphp
                      @foreach ( $rab->units as $key => $value )                        
              ...
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  ...
                  <tr>
                    <td colspan="5"><strong>{{ \Modules\Project\Entities\Templatepekerjaan::find($rab->template_pekerjaan[$i])->name }}</strong></td>                    
                  </tr>   
                  @foreach ( \Modules\Project\Entities\Templatepekerjaan::find($rab->template_pekerjaan[$i])->rab_budget as $key => $value ) 
                  <tr>
                    <td>&nbsp;</td>                    
                    <td>{{ \Modules\Pekerjaan\Entities\Itempekerjaan::find($value['itempekerjaan_id'])->name }}</td>
                    <td>{{ $value['volume'] }}</td>                    
                    <td>{{ $value['satuan'] }}</td>
                    <td>{{ $value['nilai'] }}</td>
                  </tr>     
