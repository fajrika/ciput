...
class Document
{
    public static function new_number($doc_type, $department_id) 
    {
        ...
        $project    = \Modules\Project\Entities\Project::find( session('project_id') )->code;    // use \Session::put('project', value) in command prompt
        ...
        //$count[6] = \App\Suratinstruksi::where('no','LIKE', '%'.$doc_last)->count();
        //$count[7] = \App\Vo::where('no','LIKE', '%'.$doc_last)->count();
        ...
        //$count[13] = \App\TenderKorespondensi::where('no','LIKE', '%'.$doc_last)->withTrashed()->count();
    ...
    public static function make_approval_history($id)
    {
        ...
        if ( $class == "Purchaseorder"){
            ...
            //$pt_id = \Auth::user()->project_pt_users()->whereProjectId( session('project') )->first()->pt->id; 
        ...
        //$department_from["Vo"] = $document->rab->workorder->department_from;
        
        //$department_from["Tender"] = 
